...
from dqmtools.dqmpds import df_channel_map, baseline_rms_plot

def return_obj(dash_app, engine, storage,theme):
    plot_id = "18_pds_baseline_per_channel_plot"
    ...
    def plot_pds_baseline_per_channel_grap(n_clicks, refresh, trigger_record,partition,run,raw_data_file,original_state):
        ...
        if trigger_record and raw_data_file:
           ...
           if plot_id in storage.shown_plots:
                ...
                try: fig_base_channel, fig_rms_channel = baseline_rms_plot(df)
                except RuntimeError: return(html.Div("Please choose both a run data file and trigger record"))
                    
                return(html.Div([selection_line(partition,run,raw_data_file, trigger_record),html.H4("PDS: Baseline per Channel"),dcc.Graph(figure=fig_base_channel,style={"marginTop":"10px"})]))
