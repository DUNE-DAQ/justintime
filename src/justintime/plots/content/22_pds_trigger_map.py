...
from dqmtools.dqmpds import df_channel_map, heat_map_plot

def return_obj(dash_app, engine, storage,theme):
    plot_id = "22_pds_trigger_map_plot"
    ...
    def plot_pds_baseline_per_channel_grap(n_clicks,refresh, trigger_record,partition,run,raw_data_file,original_state):
        ...
        if trigger_record and raw_data_file:
            ...
            if plot_id in storage.shown_plots:
                ...
                try: fig = heat_map_plot(df)
                except RuntimeError: return(html.Div("Please choose both a run data file and trigger record"))
                return(html.Div([selection_line(partition,run,raw_data_file, trigger_record),html.H4("PDS: Trigger Map"),dcc.Graph(figure=fig,style={"marginTop":"10px"})]))
        #return(html.Div())
       # else:
       #     return(html.Div(html.H6(nothing_to_plot())))
